...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, encoder, decoder, episode_len=20, path_type='planner_path'):
        ...
        # REMOVE: these were pFL-only placeholders and are never read by
        # the retained FedAvg or ours paths.
...
class PrefixSeq2SeqAgent(Seq2SeqAgent):
    """Seq2SeqAgent with PrefixAttnDecoderLSTM + GatePolicyTwoTower.

    The gate produces per-step scales [B, 2]:
      - [:,0]  scales the learnable context prefix  (Block 1)
      - [:,1]  scales the state adapter residual     (Block 2)

    In federated learning the encoder + decoder backbone are aggregated;
    the prefix/adapter parameters and the gate stay personal per client.
    """

    def __init__(self, env, results_path, encoder, decoder, gate_policy,
                 episode_len=20, lambda_smooth=0.01):
        super().__init__(env, results_path, encoder, decoder, episode_len)
        self.gate_policy = gate_policy
        self.lambda_smooth = lambda_smooth
        # Per-rollout diagnostics (updated after each rollout)
        self.gate_prefix_mean = 0.0
        self.gate_adapter_mean = 0.0
        self.gate_smooth_loss = torch.tensor(0.0)
        # REMOVE: per-step profiles and selected-sample JSON traces are offline
        # analysis tooling. They do not contribute to the navigation loss,
        # learned gate gradients, FedAvg aggregation, or evaluation metrics.
        # self.step_gate_profile = []
        # self.step_gate_profile_accum = {}
        # self.step_gate_profile_count = 0
        # self.gate_trace_dumped = 0
        # self.gate_trace_dumped_keys = set()

    

    # ------------------------------------------------------------------
    # Rollout with gate + smoothness loss
    # ------------------------------------------------------------------
    def rollout(self):
        obs = np.array(self.env.reset())
        batch_size = len(obs)
        ...
        self.loss = 0
        gate_values = []                      # for temporal smoothness
        env_action = [None] * batch_size

        for t in range(self.episode_len):
            f_t = self._feature_variable(perm_obs)

            # ---- Gate: per-step scales ----
            step_frac = float(t) / max(self.episode_len, 1)
            stats = torch.tensor(
                [[step_frac, 1.0 - step_frac]] * batch_size,
                device=h_t.device, dtype=torch.float32)
            g_t = self.gate_policy(h_t.detach(), stats,
                                   obs_summary=f_t.detach())  # [B, 2]
            gate_values.append(g_t)
            
            # ---- Decoder step (prefix + adapter gated) ----
            h_t, c_t, alpha, logit = self.decoder(
                a_t.view(-1, 1), f_t, h_t, c_t, ctx, seq_mask,
                gate_scales=g_t)

            # Mask forward where impossible
            for i, ob in enumerate(perm_obs):
                if len(ob['navigableLocations']) <= 1:
                    logit[i, self.model_actions.index('forward')] = -float('inf')
        ...
        self.gate_smooth_loss = torch.tensor(0.0, device=ctx.device)
        if len(gate_values) > 1 and self.lambda_smooth > 0:
            for i in range(1, len(gate_values)):
                self.gate_smooth_loss += ((gate_values[i] - gate_values[i - 1]) ** 2).mean()
            self.gate_smooth_loss /= (len(gate_values) - 1)

        # ---- Diagnostics (no grad) ----
        with torch.no_grad():
            if gate_values:
                g_stacked = torch.stack(gate_values)   # [T, B, 2]
                self.gate_prefix_mean = g_stacked[:, :, 0].mean().item()
                self.gate_adapter_mean = g_stacked[:, :, 1].mean().item()
            else:
                self.gate_prefix_mean = 0.0
                self.gate_adapter_mean = 0.0
    ...
    # Training
    # ------------------------------------------------------------------
    def train(self, backbone_opt, prefix_opt, gate_opt, n_iters,
              feedback='teacher'):
        """Train for *n_iters* rollouts.

        Three separate optimizers for:
          backbone_opt  — encoder + decoder backbone (aggregated in FL)
          prefix_opt    — decoder prefix + adapter   (personal in FL)
          gate_opt      — gate policy                (personal in FL)
        """
        assert feedback in self.feedback_options
        self.feedback = feedback
        self.encoder.train()
        ...
        for _ in range(1, n_iters + 1):
            ...
            prefix_opt.zero_grad(set_to_none=True)
            gate_opt.zero_grad(set_to_none=True)

            self.rollout()
            total_loss = self.loss + self.lambda_smooth * self.gate_smooth_loss
            total_loss.backward()
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False):
        ...
        # BaseAgent.test() with torch.no_grad wrapper
        self.env.reset_epoch()
        self.losses = []
        self.results = {}
        looped = False
    ...
    # Save / Load
    # ------------------------------------------------------------------
    def save(self, encoder_path, decoder_path, gate_path=None):
        torch.save(self.encoder.state_dict(), encoder_path)
        torch.save(self.decoder.state_dict(), decoder_path)
        if gate_path is not None:
            torch.save(self.gate_policy.state_dict(), gate_path)

    def load(self, encoder_path, decoder_path, gate_path=None):
        self.encoder.load_state_dict(torch.load(encoder_path))
        self.decoder.load_state_dict(torch.load(decoder_path))
        if gate_path is not None:
            self.gate_policy.load_state_dict(torch.load(gate_path))
