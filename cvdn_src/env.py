...
class EnvBatch():
    ...
    def __init__(self, feature_store=None, batch_size=100, blind=False):
        if feature_store:
            ...
            self.features = {}
            with open(feature_store, "r", newline="") as tsv_in_file:
                ...
                for item in reader:
                    ...
                    if not blind:
                        ...
                        self.features[long_id] = np.frombuffer(
                            base64.b64decode(item['features'].encode('ascii')),
                            dtype=np.float32
                        ).reshape((36, 2048))
                    else:
...
class R2RBatchScan():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        for item in load_datasets(splits):
            ...
            self.data[scan].append(new_item)
        self.scans = set(self.scans)
        self.splits = splits
        self.seed = seed
        random.seed(self.seed)
        ...
        self._load_nav_graphs()
        self.path_type = path_type
        self.current_scan = self.scans_list[0] if len(self.scans_list) > 0 else None
        print('R2RBatch loaded with %d instructions, using splits: %s' % (len(self.data), ",".join(splits)))
    ...
    def set_current_scan(self, scan):
        """
        scan: str (scanId) or int (index in scans_list)
        """
        if isinstance(scan, str):
            self.current_scan = scan
            self.scan_idx = self.scans_list.index(scan)
        else:
            self.scan_idx = int(scan)
            self.current_scan = self.scans_list[self.scan_idx]
        self.ix = 0
    ...
    def _next_minibatch(self, tile_one=False, batch_size=None):
        ...
        assert self.current_scan is not None, "current_scan is None, call set_current_scan() first"

        data_list = self.data[self.current_scan]
        n = len(data_list)
        assert n > 0, f"no data for scan={self.current_scan}"

        if tile_one:
            # 复制同一个样本铺满 batch（debug 用）
            batch = [data_list[self.ix]] * batch_size
            self.ix = (self.ix + 1) % n
            self.batch = batch
            return

        # 正常按顺序取 batch，不够就循环+洗牌补齐
        batch = data_list[self.ix:self.ix + batch_size]
        if len(batch) < batch_size:
            random.shuffle(data_list)          # 换一个 epoch
            remain = batch_size - len(batch)
            # remain 可能 > n（当 batch_size > n 或 ix 接近末尾时），需要 tile
            while remain >= n:
                batch += data_list[:]
                remain -= n
            if remain > 0:
                batch += data_list[:remain]
            self.ix = remain
        else:
            self.ix += batch_size
