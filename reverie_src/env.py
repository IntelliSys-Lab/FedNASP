            ...
            try:
                ...
                for vis_pos, objects in self.obj_feats[state.scanId][state.location.viewpointId].items():
                    for objId, obj in objects.items():
                        ...
                        # VilBERT object features use the four-angle representation.
                        obj_features.append(np.concatenate(
                            (obj['features'].toarray().squeeze(), directional_feature[int(vis_pos), :4]), -1))
            except KeyError:
                pass
...
class R2RBatchScan(R2RBatch):
    '''
    R2RBatch variant for federated learning that organizes data by scan.
    Supports set_current_scan() for per-client training.
    '''
    ...
    def __init__(self, feature_store, obj_feats, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        # Store data as dict: scan_id -> list of items
        self.data = {}
        self.scans_list = []
        ...
        for split in splits:
            for i_item, item in enumerate(load_datasets([split])):
                scan_id = item['scan']
                if scan_id not in ft_scans:
                    continue
                if args.test_only and i_item == 64:
                    break
                
                # Initialize scan entry if needed
                if scan_id not in self.data:
                    self.data[scan_id] = []
                    self.scans_list.append(scan_id)
                ...
                for j, instr in enumerate(item['instructions']):
                    try:
                        ...
                        if new_item['instr_encoding'] is not None:
                            self.data[scan_id].append(new_item)
                            scans.append(scan_id)
                    except:
                        continue

        # Remove empty scans
        self.scans_list = [s for s in self.scans_list if len(self.data.get(s, [])) > 0]
        ...
        # Current scan for FL training
        self.current_scan = self.scans_list[0] if self.scans_list else None
        self.scan_idx = 0
        
        total_items = sum(len(self.data[s]) for s in self.scans_list)
        print('R2RBatchScan loaded with %d instructions across %d scans, using splits: %s' % (
            total_items, len(self.scans_list), ",".join(splits)))

    def size(self):
        """Return total size across all scans."""
        return sum(len(self.data[s]) for s in self.scans_list)
    
    def current_size(self):
        """Return size of current scan's data."""
        if self.current_scan:
            return len(self.data.get(self.current_scan, []))
        return 0

    def set_current_scan(self, scan):
        """Set the current scan for FL client training."""
        if isinstance(scan, int):
            self.scan_idx = scan
            self.current_scan = self.scans_list[scan]
        else:
            self.current_scan = scan
            self.scan_idx = self.scans_list.index(scan) if scan in self.scans_list else 0
        self.ix = 0
    ...
    def _next_minibatch(self, tile_one=False, batch_size=None, **kwargs):
        ...
        current_data = self.data.get(self.current_scan, [])
        if len(current_data) == 0:
            raise ValueError(f"current_scan={self.current_scan} has no data")

        n = len(current_data)

        if tile_one:
            item = current_data[self.ix % n]
            self.batch = [item] * batch_size
            self.ix = (self.ix + 1) % n
            return

        batch = []
        while len(batch) < batch_size:
            end = min(self.ix + (batch_size - len(batch)), n)
            batch.extend(current_data[self.ix:end])
            self.ix = end
